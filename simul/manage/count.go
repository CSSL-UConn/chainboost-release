...
// Start the protocol
// Raha: This is calld in te runsimul.go in simulat function
// but I dont get which node starts the protocol
// (it should be the root nodee, but I just dont se it!)

func (p *ProtocolCount) Start() error {
	// Send an empty message
	log.Lvl2("Starting to count")
	p.FuncPC()
	return nil
}
...
func (p *ProtocolCount) Dispatch() error {
	...
	for running {
		...
		case c := <-p.CountChan:
			if len(c) == 0 {
				log.Lvl1("Raha: last element of CountMsg is nil")
			} else {
				if len(c)-1 > 0 {
					log.Lvl1("Raha: last element of CountMsg is", c[len(c)-1].Count)
				} else {
					log.Lvl1("Raha: last element of CountMsg is", c[0].Count)
				}
			}
			p.FuncC(c)
			running = false
...
func (p *ProtocolCount) FuncC(cc []CountMsg) {
	count := 1
	for _, c := range cc {
		log.Lvl1("Raha: this node", p.Info(), "has", int(c.Count.Children), "in its FuncC calling param")
		count += int(c.Count.Children)
	}
	if !p.IsRoot() {
		log.Lvl1(p.Info(), "Sends (Raha: Count ", count, " ) to", p.Parent().ID, p.Parent().ServerIdentity.Address)
		if err := p.SendTo(p.Parent(), &Count{int32(count)}); err != nil {
			log.Lvl2(p.Name(), "couldn't send to parent",
				p.Parent().Name())
		}
	} else {
		log.Lvl1("Raha: what is CountMsg now?")
		p.Count <- count
	}
	log.Lvl1(p.ServerIdentity().Address, "Done")
}
