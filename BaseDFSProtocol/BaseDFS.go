...
// (for each client we have a server who has stored his client's file
// and the storage contract is fixed for all - fixed payment)
// 2) clients have sent their escrows

// The BaseDFSProtocol goes as follows:
// 1- a server broadcast a por tx (chanllenge's randomness omes from?)
...
type ByzCoin struct {
	...
	*onet.TreeNodeInstance
	// the suite we use -- do we need do declare it here?
