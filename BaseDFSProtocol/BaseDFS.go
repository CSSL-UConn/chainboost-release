...
//checkLeadership
func (bz *BaseDFS) checkLeadership() (bool, vrf.VrfProof) {
...
func (bz *BaseDFS) createEpochBlock(ok bool, p vrf.VrfProof) {
	if ok != false {
...
func (bz *BaseDFS) readBCPreCheckLeadership() (power *big.Int, seed string) {
	...
	if success == true {
		...
		log.LLvl2("Panic Raised:\n\n error in decoding power (string) to big.int from centralbc file")
		panic(err)
	}
}
func (bz *BaseDFS) readBCForNewRound() bool {
	...
	for i, colCell := range row {
		// --- in RoundTable: i = 0 is (next) round number, i = 1 is (next) round seed, i=2 is blockchain size (empty now, will be updated by the leader)
		if i == 0 {
			lastRound, _ = strconv.Atoi(colCell)
		}
	}
...
/*	each round, adding one row in power table based on the information in market matching sheet,
	...
	for rows.Next() {
		...
		for i, colCell := range row {
			...
			if i == 0 {MinerServer = colCell}
			if i ==1  {FileSize,_ = strconv.Atoi(colCell)}
			if i ==2 {ContractDuration,_ = strconv.Atoi(colCell)}
			if i ==3 {ContractStartedRoundNumber,_ = strconv.Atoi(colCell)}
		}
	...
	log.Lvl3(ContractDuration, ContractStartedRoundNumber, FileSize,MinerServer)
	// --- Power Table sheet
	index := f.GetSheetIndex("PowerTable")
	f.SetActiveSheet(index)
	...
	} else {log.LLvl2("another leader has already published his block for round number", bz.roundNumber)}
}
