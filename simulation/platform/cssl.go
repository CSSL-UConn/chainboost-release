...
// Cssl holds all fields necessary for a Cssl-run
type Cssl struct {
	// *** Cssl-related configuration
	...
	simulDir string
	// Directory where the Cssl-users-file is held
	usersDir string
...
// for Cssl
func (d *Cssl) Configure(pcs []Config) {
...
func (d *Cssl) Deploy(rc *RunConfig) error {
	...
	log.LLvl1("Cssl: Deploying and writing config-files")
	...
	if err != nil {
		log.Fatal("error copying sidechain.db: ", err)
	}
...
// Start creates a tunnel for the monitor-output and contacts the Cssl-
// server to run the simulation
func (d *Cssl) Start(args ...string) error {
	return nil
}

// Wait for the process to finish
func (d *Cssl) Wait() error {
	return nil
}
...
func (d *Cssl) loadAndCheckCsslVars() {
	deter := Cssl{}
	...
	if d.Host == "" {
		d.Host = readString("Please enter the hostname of Cssl", "csi-lab-ssh.engr.uconn.edu")
	}
	...
	if d.Project == "" {
		d.Project = readString("Please enter the project on Cssl", "SAFER")
	}
