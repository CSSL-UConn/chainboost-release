...
func (s *SimulationProtocol) Node(config *onet.SimulationConfig) error {
		...
		if n.ID.Equal(config.Server.ServerIdentity.ID) {
			config.Server.RegisterProcessorFunc(onet.ProtocolMsgID, func(e *network.Envelope) error {
				...
				case *protocol.Announcement, *protocol.Response:
					//log.LLvl3("Ignoring blscosi message for simulation on ", config.Server.ServerIdentity)
				default:
	...
	//log.LLvl3("Initializing node-index", index)
...
func (s *SimulationProtocol) Run(config *onet.SimulationConfig) error {
	...
	//log.LLvl3("Size is:", size, "rounds:", s.Rounds)
	for round := 0; round < s.Rounds; round++ {
		//log.LLvl3("Starting round", round)
		...
		//log.LLvl3("Signature correctly verified!")
	}
	return nil
}
